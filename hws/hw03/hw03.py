...
def count_coins(total):
    ...
    def constrained_count(total, largest_coin):
        if total == 0:
            return 1
        if total < 1:
            return 0
        if largest_coin == None:
            return 0
        without_coin = constrained_count(total, next_smaller_coin(largest_coin))
        with_coin = constrained_count(total - largest_coin, largest_coin)
        return with_coin + without_coin
    
    return constrained_count(total, 25)
